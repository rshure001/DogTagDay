...
const recordByEmail = new Map((message.records || []).map(record => [record.email.toLowerCase(), record]));

let submitted = 0, skipped = 0, failed = 0;
for (const to of recipients) {
  try {
    const prepared = recordByEmail.get(to);
    const result = await sendOne({
      to,
      subject: prepared?.subject || message.subject,
      text: prepared?.body || message.body,
