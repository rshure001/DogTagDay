(() => {
  ...
  // Remove any older holiday badge/pill that may have been injected by a cached script.
  hero.querySelectorAll('[data-dtd-holiday], [aria-label="Labor Day"]').forEach(el => el.remove());
  ...
  const holidays = [
    ...
    {name:'Halloween', date:new Date(y,9,31), icon:'🎃'}
  ];
  ...
  accent.dataset.dtdHoliday = 'subtle';
  accent.setAttribute('aria-label', current.name);
  accent.title = current.name;
  accent.textContent = current.icon;
  accent.style.cssText = 'position:absolute;right:10px;top:10px;z-index:8;width:30px;height:30px;display:flex;align-items:center;justify-content:center;border-radius:50%;background:rgba(5,12,20,.38);border:1px solid rgba(217,180,90,.45);box-shadow:0 2px 8px rgba(0,0,0,.18);font-size:18px;line-height:1;pointer-events:none';
