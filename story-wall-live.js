...
(() => {
  ...
  if (wall) fallbackStories.slice().reverse().forEach(s => renderStory(s, true));
  ...
  form.addEventListener('submit', async e => {
    ...
    if (!text) {
      status.textContent = 'Please enter your story.';
      message?.focus();
      return;
    }
    if (submit) {
      submit.disabled = true;
      submit.textContent = 'Posting Story…';
    }
    ...
    try {
      ...
      if (submit) {
        ...
        submit.textContent = 'Post My Story';
      }
    }
  });
})();
