...
storyForm?.addEventListener('submit',async event=>{
  ...
  if(storySubmit){storySubmit.disabled=true;storySubmit.textContent='Sending Story…';}
  if(storyWarning){storyWarning.textContent=`Saving your story — receipt ${receipt}`;storyWarning.style.fontWeight='700';}
  try{
    ...
    storyForm.reset();
    if(storyWarning){storyWarning.textContent=`Story received. Save this receipt: ${receipt}. Your story was sent successfully to Dog Tag Day.`;storyWarning.style.fontWeight='700';}
  }catch(error){
  ...
  }finally{if(storySubmit){storySubmit.disabled=false;storySubmit.textContent='Add My Story';}}
});
...
if(storySection&&storyWall){indexExistingStories();const lead=storySection.querySelector('.story-lead');if(lead)lead.textContent='Real stories shared with permission. Dog Tag Day is building this national story wall one veteran, one family, and one memory at a time.';const controls=document.createElement('div');controls.style.margin='28px 0 18px';searchBox=document.createElement('input');searchBox.type='search';searchBox.placeholder='Search stories by name or keyword';searchBox.setAttribute('aria-label','Search community stories');searchBox.style.width='100%';searchBox.style.maxWidth='620px';searchBox.style.padding='14px 16px';searchBox.style.borderRadius='8px';searchBox.style.border='1px solid currentColor';controls.appendChild(searchBox);storyCount=document.createElement('p');storyCount.style.margin='8px 0 20px';storyCount.style.fontWeight='700';storyCount.textContent=`${storyWall.querySelectorAll('article').length} published stories — every story on this wall was shared with permission.`;loadMore=document.createElement('button');loadMore.type='button';loadMore.className='button secondary';loadMore.textContent='Load More Stories';loadMore.style.marginTop='20px';loadMore.hidden=true;storyWall.before(controls,storyCount);storyWall.after(loadMore);}
...
async function loadStories(reset=false){if(!storyWall)return;try{const page=reset?1:storiesPage;const url=`https://api.github.com/repos/rshure001/DogTagDay/issues?state=open&labels=formspree&sort=created&direction=desc&per_page=${storiesPerPage}&page=${page}`;const response=await fetch(url,{headers:{Accept:'application/vnd.github+json'}});if(!response.ok)throw new Error(`GitHub ${response.status}`);const issues=(await response.json()).filter(issue=>!issue.pull_request);issues.forEach(renderIssue);if(storyCount){const count=storyWall.querySelectorAll('article').length;storyCount.textContent=`${count} published ${count===1?'story':'stories'} — every story on this wall was shared with permission.`;}if(loadMore)loadMore.hidden=issues.length<storiesPerPage;if(issues.length===storiesPerPage)storiesPage=page+1;filterStories();}catch(error){if(storyCount){const count=storyWall.querySelectorAll('article').length;storyCount.textContent=`${count} published ${count===1?'story':'stories'} — verified stories remain available even if the live feed is temporarily unavailable.`;}console.error('Story wall load failed',error);}}
loadMore?.addEventListener('click',()=>loadStories(false));loadStories(true);
if(storyForm){const formEyebrow=storyForm.querySelector('.eyebrow');if(formEyebrow)formEyebrow.textContent='Add a Story to the National Wall';const formHeading=storyForm.querySelector('h3');if(formHeading)formHeading.textContent='Tell us the story behind the tags.';const connectionInput=storyForm.querySelector('input[name="connection"]');if(connectionInput)connectionInput.placeholder='Veteran, family member, friend, supporter — optional';if(storyText)storyText.placeholder='A few sentences is enough. Tell us who served, what the tags mean, or who you want America to remember.';if(storySubmit)storySubmit.textContent='Add My Story';}
